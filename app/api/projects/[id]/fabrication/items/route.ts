...
import {createProjectFabricationItemSchema} from "@/app/core/projects/fabrication/schema";
...
export async function GET(req: Request, ctx: {params: Promise<{id: string}>}) {
  try {
    ...
    const items = await projectFabricationPrismaRepo.listItems(fabrication.id);
...
export async function POST(req: Request, ctx: {params: Promise<{id: string}>}) {
  try {
    ...
    const parsed = createProjectFabricationItemSchema.parse(body);

    const uc = makeProjectFabricationUseCases(projectFabricationPrismaRepo);
    const fabrication = await uc.getProjectFabrication.execute(projectId);

    const result = await uc.createProjectFabricationItem.execute({
      ...parsed,
